      ...
      <vs-col vs-type="flex" vs-justify="center" vs-align="center" class="m-5 w-1/4 md:w-1/4 sm:w-1">
        <vs-card class="border-solid border-primary fixedHeight">
          <div slot="header" class="flex justify-between align-middle">
            ...
            <div>
              <feather-icon icon="PlusIcon" svgClasses="w-5 h-5 hover:text-success stroke-current"
                            class="cursor-pointer"
                            @click="addNewData(0,1)"/>
            </div>
          </div>
          <vs-list class="height-300">
            <ul>
              <li v-for="(item , index) in firstLevelCategories" :key="index" class="cursor-pointer hover:bg-primary"
                  :class="[(firstLevelActiveIndex == item.id) ? 'active':'']"
                  @click="fetch_second_level_category(item.id)">
        ...
        <vs-card class="border-solid border-primary fill-height">
          <div slot="header" class="flex justify-between align-middle">
            ...
            <div v-if="firstLevelActiveIndex">
              <feather-icon icon="PlusIcon" svgClasses="w-5 h-5 hover:text-success stroke-current"
                            class="cursor-pointer"
                            @click="addNewData(firstLevelActiveIndex,2)"/>
            </div>
          </div>
          <vue-perfect-scrollbar>
            <vs-list class="height-300">
              <ul>
                <li v-for="(item , index) in secondLevelCategories" :key="index" class="cursor-pointer hover:bg-primary"
                    :class="[(secondLevelActiveIndex == item.id) ? 'active':'']"
                    @click="fetch_third_level_category(item.id)">
                  <vs-list-item icon="check" :title="item.title">
                    <feather-icon icon="SlackIcon" svgClasses="w-5 h-5 hover:text-success stroke-current"
                                  @click="attrRoute(item.id)"/>
                    <feather-icon icon="EditIcon" svgClasses="w-5 h-5 hover:text-warning stroke-current"
        ...
        <vs-card class="border-solid border-primary ">
          <div slot="header" class="flex justify-between align-middle">
            ...
            <div v-if="secondLevelActiveIndex">
              <feather-icon icon="PlusIcon" svgClasses="w-5 h-5 hover:text-success stroke-current"
                            class="cursor-pointer"
                            @click="addNewData(secondLevelActiveIndex,3)"/>
            </div>
          </div>
          <vs-list class="height-300">
            <ul>
...
export default {
  ...
  computed: {},
  methods: {
    addNewData (parent_id, level) {
      this.sidebarData = {parent_id,level}
