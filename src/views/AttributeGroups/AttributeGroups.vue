        ...
        <vx-card ref="filterCard" title="جست و جو" class="roles-list-filters mb-8">
            <div class="vx-row">
                    <div class="vx-col md:w-1/2 sm:w-1/2 w-full">
                        <label for="roleName" class="text-sm opacity-75" >نام</label>
                        <vs-input id="roleName" v-model="attributeGroupNameSearch" class="w-full mt-4" placeholder="نام"/>
                    </div>
            </div>
        </vx-card>
        <data-view-sidebar :isSidebarActive="addNewDataSidebar" @closeSidebar="toggleDataSidebar" :data="sidebarData"/>
        <vx-card actionButtons title="همه نقش ها" class="roles-list mb-8">
            <vs-table ref="table" multiple v-model="selected" pagination :max-items="itemsPerPage"  :data="resultQuery" noDataText="موردی برای نمایش وجود ندارد">
                ...
                <span class="flex items-center">
                  ...
                  <span>حذف</span>
                        ...
                        <button class="p-3 mb-4 mr-4 rounded-lg cursor-pointer flex items-center justify-center text-lg font-medium text-base
                          text-primary border border-solid border-primary " style="background-color: #fff" @click="addNewData">
                    ...
                    <vs-dropdown vs-trigger-click class="cursor-pointer mb-4 mr-4 items-per-page-handler">
                        <div
                            ...
                            <span class="mr-2">{{ currentPage * itemsPerPage - (itemsPerPage - 1) }} - {{ resultQuery.length - currentPage * itemsPerPage > 0 ? currentPage * itemsPerPage : resultQuery.length }} از {{ queriedItems }} آیتم</span>
                            <feather-icon icon="ChevronDownIcon" svgClasses="h-4 w-4"/>
                        </div>
...
<script>
    import DataViewSidebar from './AttributeGroupsSidebar'
    ...
    export default {
        ...
        computed: {
            ...
            queriedItems() {
                return this.$refs.table ? this.$refs.table.queriedResults.length : this.attributeGroups.length
            },
            resultQuery(){
                if(this.attributeGroupNameSearch !=''){
                    return this.attributeGroups.filter((item)=>{
                        return this.attributeGroupNameSearch.toLowerCase().split(' ').every(v => item.name.toLowerCase().includes(v))
                    })
                }else{
        ...
        methods: {
            ...
            deleteRecord(id) {
                axios.delete(`attributegroups/`+id)
                    .then((response) => {
                        this.attributeGroups=response.data.attributeGroups
                        this.$vs.notify({
