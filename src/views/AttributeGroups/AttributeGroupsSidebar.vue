<template>
    <vs-sidebar click-not-close position-right parent="body" default-index="1" color="primary"
        ...
        <div class="mt-6 flex items-center justify-between px-6">
            <h4>{{ Object.entries(this.data).length === 0 ? "افزودن " : "ویرایش" }} گروه ویژگی</h4>
        ...
        <VuePerfectScrollbar class="scroll-area--data-list-add-new" >
            <div class="p-6">
                <vs-input label="عنوان" v-model="dataTitle" class="mt-5 w-full" name="item-name" v-validate="'required'"/>
        ...
        watch: {
            isSidebarActive(val) {
                ...
                if (Object.entries(this.data).length === 0) {
                    ...
                    let {id, title} = JSON.parse(JSON.stringify(this.data))
                    this.dataId = id
                ...
                this.$validator.validateAll().then(result => {
                    if (result) {
                      ...
                      formData.append('title',this.dataTitle)
                        if (Object.entries(this.data).length === 0 ){
                            var customUrl = "attributegroups"
                        }
                        else {
                          formData.append('_method', 'patch')
                            customUrl = "attributegroups/" + this.dataId
                        }
                        axios.post(customUrl,formData)
                            .then((response) => {
                                this.$parent.attributeGroups = response.data.attributeGroups
                                this.$vs.notify({
