<template>
    <vs-sidebar click-not-close position-right parent="body" default-index="1" color="primary"
        ...
        <VuePerfectScrollbar class="scroll-area--data-list-add-new" >
            <div class="p-6">
                <vs-input label="نقش" v-model="dataName" class="mt-5 w-full" name="item-name" v-validate="'required'"/>
                <span class="text-danger text-sm" v-show="errors.has('item-name')">{{ errors.first('item-name') }}</span>
                <vs-input label="عنوان" v-model="dataDescription" class="mt-5 w-full" v-validate="'required'" name="description"/>
        ...
        watch: {
            isSidebarActive(val) {
                ...
                if (Object.entries(this.data).length === 0) {
                    ...
                    let {id, name, description} = JSON.parse(JSON.stringify(this.data))
                    this.dataId = id
